---
description: "Web GUI 的外壳布局：三栏 AppFrame——其右栏是贴边面板的轨道——其移动端单视图呈现、面板几何服务与主题呈现；供窗口外观的用户与维护者阅读。"
...
本包提供 Web GUI 的三栏 AppFrame、左右栏宽度与 `ctx.layout` 呈现控制。右栏先让步以保护中栏空间，全屏由占用方呈现，框架保留宽屏底层轨道。宽度不超过 768px 时，同样的占用方保持挂载，但一次只显示一个整宽视图：会话列表或对话。主题呈现器负责配色、别名 token、正文字号与 document 元数据；布局状态在刷新后重置。
...
宽度不超过 768px 时，框架不再是分栏布局。侧栏与对话共用一个整宽单元并保持挂载，因此两者之间切换会保留滚动位置与草稿；右栏无法占用轨道，两个拖拽区都不显示。打开一个并非当前会话的会话会显示对话，从列表中重新选中当前会话同样如此；随会话列表恢复的选择不是导航手势，因此重新加载停在列表。一条 40px、带会话标题与返回控件的条用于回到列表。所选视图在双向跨越断点后仍然保留。框架自身的外围到此为止；各栏占用方各自收紧其手机呈现，好让记录区占住屏幕。
...
- **极窄窗口**——在 769px 与中栏所需宽度之间，右栏关闭后左侧 56px 控制栏仍保留；低于 769px 时框架改用单视图呈现。
- **重新选中当前会话依赖一个 ARIA 关系**——移动端的点击信号把带 `aria-selected` 的树项识别为会话行。若侧栏不再这样标记行，普通的会话切换仍会切换视图，重新选中则不会。
