...
宽度不超过 768px 时，框架不再是分栏布局。侧栏与对话共用一个整宽单元并保持挂载，因此两者之间切换会保留滚动位置与草稿；右栏无法占用轨道，两个拖拽区都不显示。打开一个并非当前会话的会话会显示对话，从列表中重新选中当前会话同样如此；随会话列表恢复的选择不是导航手势，因此重新加载停在列表。一条 44px、带会话标题与返回控件的条用于回到列表。所选视图在双向跨越断点后仍然保留。
