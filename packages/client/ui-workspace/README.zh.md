...
搁置会在不触碰历史的前提下把 Session 放到一边：它离开分组与扁平列表，进入固定在其下方的**已搁置**分区，配色变淡且默认折叠。没有任何搁置项时该分区不渲染；达到列高三分之一后自带滚动；搜索结果中不出现。打开搁置行可以正常阅读且保持搁置；向其发送消息会取消搁置，行内的**取消搁置**操作同样如此。恢复后的 Session 回到其 Workspace 账本仍然保留的位置。
