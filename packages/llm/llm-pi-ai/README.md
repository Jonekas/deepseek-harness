...
Completed `openai-codex-responses` answers and compaction summaries are not rejected solely because reported input exceeds catalog capacity. Empty stops still fail with `EMPTY_RESPONSE`; explicit provider overflow and zero-output length overflow retain `CONTEXT_WINDOW_EXCEEDED`. Other protocols retain pi-ai's usage-based overflow detection. Catalog capacity still controls proactive compaction; this does not raise the route's supported limit. See the [Codex completion decision](../../../.agents/notes/implemented/bug-fix/2026-09-08-codex-completion-capacity.md).
