...
已完成的 `openai-codex-responses` 答案和压缩摘要不会仅因报告的输入超过目录容量而被拒绝。空 stop 仍以 `EMPTY_RESPONSE` 失败；显式提供方溢出与零输出长度溢出仍归为 `CONTEXT_WINDOW_EXCEEDED`。其他协议保留 pi-ai 基于用量的溢出检测。目录容量仍控制主动压缩；这不会提高路由支持的限制。参见 [Codex 补全决策](../../../.agents/notes/implemented/bug-fix/2026-09-08-codex-completion-capacity.zh.md)。
