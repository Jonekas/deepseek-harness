...
`archiveSession` 与 `restoreSession` 是对该集合的两个写入操作，它们与其他所有注册表写入运行在同一条串行化操作链上。归档要求会话在活动注册表或持久化层中已知；恢复只要求集合成员身份，而这一点归档时已经证明，因此持久化列举失败绝不会困住一次恢复。
...
- **恢复是无条件的**——`restoreSession` 只移除集合成员身份，因此不做会话查找，id 从未被归档时也不会报告任何信息；调用方若想确认会话是否存在，必须自行询问持久化层。
