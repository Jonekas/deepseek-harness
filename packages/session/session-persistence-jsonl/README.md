...
- **Deleted sessions occupy disk until their retention expires** — `delete` renames the session directory into `root/_trash` and a later delete discards entries older than `trashRetentionDays` (default 30), so space is reclaimed on the next deletion rather than immediately; an entry whose name this build cannot parse is never reclaimed, and emptying the area on demand is a manual operation.
