...
- **The Settled section** is the last child of whichever list body is showing, so it sits directly under the threads and scrolls with them rather than floating at the column foot above empty space. It is collapsed by default and absent entirely when nothing is settled. Both bodies receive it as one plain `SettledSectionProps` value rather than a rendered node, keeping UI data crossing component seams as data and callbacks. Its expansion rides `groupExpansion` under the reserved `SETTLED_GROUP_KEY`; a new top-level store field would have read `undefined` from every persisted store written before it existed, because rehydration replaces state wholesale.
...
- **Pinning the section to the foot of the sidebar column.** Shipped first, then replaced: with a short thread list it detached from the threads and floated over empty space, reading as sidebar chrome rather than as the end of the list. Anchoring it under the threads costs it a fixed position when the list is long, which the collapsed default keeps cheap.
